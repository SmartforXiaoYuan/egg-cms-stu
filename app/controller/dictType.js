...
class DictTypeController extends Controller {
  ...
  async show() {
    ...
    const result = await service.dictType.findByPk(id)
    ctx.returnBody(result, 100010)
  }
  ...
  async create() {
    ...
    const result = await service.dictType.create(query)
    if (result) {
  ...
  async update() {
    ...
    const result = await service.dictType.update(query, {
      id,
    })
    if (result) {
  ...
  async destroy() {
    ...
    const result = await service.dictType.destroy(ids)
