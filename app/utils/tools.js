...
function getDeptWhere(ctx, where = {}) {
  let dataScopeList = ctx.state.user.roles.map((item) =>
    parseInt(item.dataScope)
  )
  ...
  if (dataScope === 1) {
    ...
    if (where.deptId) {
      where.deptId = {
        [Op.or]: getIdLists(
          ctx.state.departmentsObj.obj[where.deptId].children,
          [where.deptId],
          'deptId'
        ),
      }
      ...
      where.deptId = {
        [Op.or]: getIdLists(
          ctx.state.departmentsObj.obj[ctx.state.user.deptId].children,
          [ctx.state.user.deptId],
          'deptId'
        ),
      }
    }
